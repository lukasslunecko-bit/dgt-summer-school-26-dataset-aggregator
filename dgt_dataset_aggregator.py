...
BACKUP_ROOT_NAME = "05-automatic-correction-backups"
...
def scan_candidates(project_data: Path) -> tuple[list[Candidate], list[AttentionItem], int]:
    ...
    for current, dirs, files in os.walk(project_data):
        ...
        for name in sorted(files, key=str.casefold):
            if not name.casefold().endswith((".csv", ".tsv")):
                continue
            scanned += 1
...
def build_aggregate_groups(
    ...
    seed: str,
) -> dict[str, dict[str, list[AggregateRow]]]:
...
def write_aggregate_outputs(
    ...
    for location, folder_name in LOCATION_FOLDERS.items():
        ...
        for group, filename in AGGREGATE_FILENAMES.items():
            ...
            if rows:
                atomic_write_bytes(path, csv_bytes((row.segment, row.ori, row.tra) for row in rows))
            elif path.exists():
                path.unlink()
            report_rows.append({
...
def aggregate_project(
    ...
    result = ProjectResult(project_data=project_data, status="running")
    log(f"Scanning {project_data}")
    candidates, scan_attention, scanned = scan_candidates(project_data)
    ...
    by_location = build_aggregate_groups(
    ...
    report_rows, provenance = write_aggregate_outputs(project_data, by_location)
    result.report_rows = report_rows
    result.status = "complete"
    attention_rows = [asdict(item) for item in result.attention]
    write_dict_csv(
        output_root / REPORT_CSV_NAME,
        ("record_type", "location", "aggregate_file", "segments", "source_files", "created", "severity", "path", "issue", "action"),
...
def aggregate_projects(
    ...
    folders = find_project_data_folders(selected)
    return [
        aggregate_project(
            folder,
...
def run_cli(args: argparse.Namespace) -> int:
    if not args.project_folder:
        print("error: at least one --project-folder is required in headless mode", file=sys.stderr)
        return 2
    try:
        results = aggregate_projects(
            args.project_folder,
            args.verification_percent,
            args.seed,
            args.auto_fix,
            args.changed_file_action,
            console_decision_provider,
            print,
            args.dry_run,
        )
    except AggregationError as exc:
...
def run_gui(default_folders: Sequence[Path] = ()) -> int:
    try:
    ...
    class AggregatorWindow(tk.Tk):
        ...
        def decide(self, kind: str, context: dict[str, object]) -> str:
            ...
            if kind == "automatic_correction":
                ...
                answer = messagebox.askyesno(
                    ...
                    f"{context.get('path')}\n\n{corrections}\n\n"
                    "The original will be copied to 05-automatic-correction-backups before correction.",
                    parent=self,
                )
        ...
        def run(self, dry_run: bool) -> None:
            ...
            if not folders:
                messagebox.showerror("No folders", "Add at least one project folder.", parent=self)
                return
            try:
            ...
            try:
                results = aggregate_projects(
                    ...
                    percent,
                    self.seed_var.get(),
                    self.auto_fix_var.get(),
                    self.changed_var.get(),
                    self.decide,
                    self.log,
                    dry_run,
                )
                ...
                messagebox.showinfo(
                    ...
                    "See each 4-Aggregated-datasets report for details." if not dry_run else
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.headless or args.project_folder:
        return run_cli(args)
    return run_gui()
